...
export function RelatedCourses({ currentCourseId, category, limit = 3 }: RelatedCoursesProps) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (category) {
          coursesQuery = query(
            ...
            equalTo(category),
            limitToFirst(limit + 1), // +1 to account for filtering out current course
          )
        } else {
          coursesQuery = query(ref(db, "courses"), limitToFirst(limit + 5)) // Get more to filter
        }
    ...
    fetchRelatedCourses()
  }, [db, currentCourseId, category, limit])
    ...
    return (
      <Card className="overflow-hidden border-0 shadow-md">
        <CardContent className="p-6 text-center">
          <p className="text-gray-600 dark:text-gray-400">Холбоотой хичээлүүд олдсонгүй.</p>
        </CardContent>
      </Card>
    )
  }
